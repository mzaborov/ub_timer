...
| **Сгенерировать для пустых строк** | Обход всего листа; запись только в пустые `SituationDescription` / `SituationRoles` |
...
- **Классика / Парный** — `SituationRoles` с полями `Role` и `Goals`; нужны «Полное описание» и «Роли и интересы».

- **Экспресс** — ровно 2 роли с полем `Phrase`; «Роли и интересы» могут быть пустыми. В `SituationDescription` через `<strong>` выделяются роли и агрессивные реплики (в кавычках / после тире); `Phrase` в JSON совпадает с репликой в тексте.
