...
│   └── init.js             # Инициализация: донаты, initTimers(), стили голосов, звук и список треков интро
│
├── css/
...
| **Hidden** | *(необязательно)* Скрыть название ситуации в выпадашке «Выберите поединок» и в заголовках колонок таблицы «Судьи» до выбора поединка: `1`, `да`, `true` и т.п. Пустая ячейка или отсутствие колонки — название видно сразу (как в старых файлах без этой колонки). |

**Отображение имён в UI:** для Классики — `Игрок (Секундант)`; для Парного — `Игрок + Секундант` (логика часов как у Классики); для Экспресса — только игрок. В таблице «Судьи» имена по-прежнему по одному в ячейке; для парных поединков строки секундантов подписываются «Секундант/игрок» и подсвечиваются как игроки.
...
- Таблица: строки — роли (Игрок, Секундант, Нанимающиеся 1…, Отправляющие 1…, Доверяющие 1…); колонки — поединки. Количество судейских строк зависит от типа поединка (классика/экспресс) и числа судей (5/7/9). В **смешанном** расписании всегда пять строк «Отправляющие 1…5»: в классике активны 1–3, в экспрессе — все пять; «Нанимающиеся» и «Доверяющие» серые только в колонках экспресса.
