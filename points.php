 ...
 * URL: points.php (вход по cookie ub_me). Старый rating.php — 410 Gone.
...
function rating_portal_who_url() {
    return 'https://ciocdo-org-skills.zaborov.ru/?p=profile';
}

function rating_echo_gate() {
    $who = rating_portal_who_url();
    echo '<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8">';
    echo '<meta name="viewport" content="width=device-width, initial-scale=1">';
    echo '<meta name="robots" content="noindex, nofollow">';
    echo '<title>Количество баллов</title>';
    echo '<style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;margin:0;background:#fff;color:#333}';
    echo 'header{background:#e65100;color:#1a1a1a;padding:.6rem .75rem}h1{margin:0;font-size:1.15rem}';
    echo 'main{padding:1.25rem .75rem;max-width:36rem}a{color:#bf360c}';
    echo '.btn{display:inline-block;margin-top:.6rem;padding:.45rem .8rem;background:#e65100;color:#1a1a1a;font-weight:700;text-decoration:none;border-radius:6px}</style>';
    echo '</head><body><header><h1>Количество баллов</h1></header><main>';
    echo '<p>Рейтинг виден только участникам. Представьтесь на портале — «Кто вы?».</p>';
    echo '<p><a class="btn" href="' . htmlspecialchars($who, ENT_QUOTES, 'UTF-8') . '">Кто вы?</a></p>';
    echo '<p><a href="index.html">Часы</a> · <a href="situations-bank.html">Банк ситуаций</a></p>';
    echo '</main></body></html>';
}
...
$meOk = false;
$chk = mysql_query("SELECT id FROM people WHERE id = " . (int)$meId, $link);
if ($chk && mysql_fetch_assoc($chk)) {
    $meOk = true;
}
if (!$meOk) {
    rating_echo_gate();
